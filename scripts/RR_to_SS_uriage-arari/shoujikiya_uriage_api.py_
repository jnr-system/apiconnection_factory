...
# ★★★ 集計期間をここで設定してください ★★★
START_DATE = "2026/01/01"   # 集計開始日 (YYYY/MM/DD)
END_DATE   = "2026/03/11"   # 集計終了日 (YYYY/MM/DD)
...
LOG_FILE_NAME = "execution_csv_log_shoujikiya.txt"
...
DB_CONFIG = {
    ...
    "details_id": "101185",
    "targets": [
        {"label": "4月-9月施工分", "view_id": "107434", "list_id": "101360"}
    ],
    "keywords": {
    ...
    "contract_targets": [  # 成約件数と成約請求金額のカウント用
        {"label": "4月-9月成約分", "view_id": "107440", "list_id": "101360"}
    ],
    "total_settings": {
        "enabled": True,
...
def main():
    ...
    try:
        ...
        write_log(f"エラー: 日付フォーマットが不正です。YYYY/MM/DD 形式で入力してください。({e})")
        return
    ...
    processed_contract_ids = set() # 重複カウント防止用
    ...
    # ── 成約日専用データ取得 ──
    extra_contract_store = {}  # 当月完了/次月繰越/再工事 を成約日キーで集計
    for c_target in db_config.get("contract_targets", []):
                ...
                if not rows_c:
                    ...
                    if c_idx_date == -1:
                        ...
                        for row in rows_c[1:]:
                            ...
                            # 進捗チェック (重複以外)
                            val_status_c = row[c_idx_status].strip().replace('"', '').replace("'", "") if c_idx_status != -1 else ""
                            if val_status_c == "重複":
                                continue
                            is_cancel_c = "キャンセル" in val_status_c

                            val_req_c = row[c_idx_req].strip().replace('"', '').replace("'", "") if c_idx_req != -1 else ""
                            ...
                            if val_type_c:
                                for cat in db_config["category_settings"]:
                                    if cat["keyword"] in val_type_c:
                                        matched_cat = cat["label"]; break

                            # extra_contract_store 初期化ヘルパー
                            ...
                            # 3. 有効な成約 (施工/空白/施工不可 かつ 金額あり)
                            elif val_req_c in ("施工", "", "施工不可") and val_bill != 0:
                                _ensure_contract("total")
                                contract_store[fmt_cdate]["total"]["billing"] += val_bill
                                if matched_cat:
                                    _ensure_contract(matched_cat)
                                    contract_store[fmt_cdate][matched_cat]["billing"] += val_bill
                                # contract_store[fmt_cdate]["count"] += 1  <-- ここでの一律加算を廃止し、下部で条件付き加算に変更
                                ...
                                if is_complete_c:
                                    ...
                                    contract_store[fmt_cdate]["total"]["count"] += 1 # ★施工日未定も成約件数に加算
    ...
    for ym in sorted(target_months):
        ...
        # ── 成約日ベース（当月完了/次月繰越/当月再工事/次月再工事）の書き込み ──
