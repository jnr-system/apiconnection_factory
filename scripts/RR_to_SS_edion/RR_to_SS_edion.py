...
def main():
    ...
    try:
        ...
        spreadsheet_headers = [
            ...
            "顧客電話番号", "顧客メールアドレス", "顧客住所", "既設商品", "ガス種",
            "先方店舗名", "先方担当者名", "工事第１希望日",
    ...
    # ------------------------------------------------------------------
    try:
        existing_values  = master_ws.get_all_values()  # 1行目はヘッダー
        existing_row_count = len(existing_values)       # ヘッダー含む現在の行数

        # C列（index=2）からレコードIDをセットに格納
        existing_record_ids = set()
        for row in existing_values[1:]:  # 2行目〜
    ...
    # ------------------------------------------------------------------
    try:
        next_row = existing_row_count + 1  # 既存行数+1 が書き込み開始行
